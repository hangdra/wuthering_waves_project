...
all_template_name = ["e_forte_not_ready","e_forte","mouse_forte","mouse_forte_not_ready","char_1","char_2","char_3"]
...
for filename in os.listdir(project_root / output_dir):
    ...
    for template_name in all_template_name:
        ...
        res = my_LoadAllTemplate_instance.has_template_by_name(tar_img,template_name)
        time_e = time.perf_counter()
        print(f"get_match_char_template_head_icon time cost : {(time_e-time_s):.4f}")
        image_template_result[template_name] = res[0]
